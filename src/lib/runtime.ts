...
//
// Persistence model on HF Spaces:
//   1. HF Space Secrets (set in Space Settings UI) → process.env.* → PERSIST
//      across restarts/rebuilds. This is where API keys + Supabase creds go.
//   2. HF Storage Bucket mounted at /data → PERSISTS across restarts.
//      This is where the SQLite DB goes (DATABASE_URL=file:/data/custom.db).
//   3. /app (container filesystem) → EPHEMERAL, wiped on every rebuild.
//      Build-time seeding alone loses all user data on each deploy.
 ...
 * Bucket mount point). Locally, falls back to the project root. The SQLite
 * DB + any other persistent files should live here.
 */
...
export function getDatabaseUrl(): string {
  // If DATABASE_URL is explicitly set (HF Secret/Variable), use it as-is.
  if (process.env.DATABASE_URL) return process.env.DATABASE_URL;
  // On HF Spaces, default to the persistent /data volume.
  if (IS_HF_SPACE) return 'file:/data/custom.db';
  // Local dev: project-relative.
 ...
 *   APP_PASSWORD              — app lock password (overrides default "omniscient")
